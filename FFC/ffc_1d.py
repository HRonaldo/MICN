...
class BasicBlock_1d(nn.Module):
    ...
    def __init__(self,inplanes:int ,stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, ratio_gin=0.5, ratio_gout=0.5, lfu=True, use_se=True, norm_layer=None,
                 ):
        ...
        width = inplanes // self.expansion
        # Both self.conv2 and self.downsample layers downsample the input when
        # stride != 1
        self.conv1 = FFC_BN_ACT_1d(inplanes, width, kernel_size=3, padding=1, stride=stride,
                                ratio_gin=ratio_gin, ratio_gout=ratio_gout, norm_layer=norm_layer, activation_layer=nn.Tanh, enable_lfu=lfu)
        self.conv2 = FFC_BN_ACT_1d(width, width * self.expansion, kernel_size=3, padding=1,
                                ratio_gin=ratio_gout, ratio_gout=ratio_gout, norm_layer=norm_layer, enable_lfu=lfu)
        self.se_block = FFCSE_block_1d(
            width * self.expansion, ratio_gout) if use_se else nn.Identity()
        self.act_l = nn.Identity() if ratio_gout == 1 else nn.Tanh()
        self.act_g = nn.Identity() if ratio_gout == 0 else nn.Tanh()
    ...
    def forward(self, x):
        ...
        x_l = self.act_l(x_l + id_l)
        x_g = self.act_g(x_g + id_g)

        return x_l, x_g


class FFCSE_block_1d(nn.Module):

    def __init__(self, channels, ratio_g):
        super(FFCSE_block_1d, self).__init__()
