...
class MIC(nn.Module):
    ...
    """
    def __init__(self, patch_len:int,patch_num:int,pred_patch_num:int,feature_size=512, n_heads=8, dropout=0.05, decomp_kernel=[32], conv_kernel=[24], isometric_kernel=[18, 6], device='cuda'):
        super(MIC, self).__init__()
        self.src_mask = None
        self.conv_kernel = [i//patch_len for i in conv_kernel]
        ...
        for ii in self.conv_kernel:
            if ii % 2 == 0:  # the kernel of decomposition operation must be odd
                ...
                temp = (patch_num+2*(ii//2)-ii)//ii +1
                isometric_kernel.append(int(temp))
                # isometric_kernel.append((patch_num + pred_patch_num + ii) // ii)
                # isometric_kernel.append(int((2*patch_num-1)-(patch_num+2*(ii/2)-ii)//ii))
            else:
                decomp_kernel.append(ii)
                temp = (patch_num+2*(ii//2)-ii)//ii +1
                isometric_kernel.append(int(temp ))
                # isometric_kernel.append(int((2*patch_num-1)-(patch_num+2*(ii/2)-ii)//ii))
                # isometric_kernel.append((patch_num + pred_patch_num + ii - 1) // ii)
...
class Seasonal_Prediction(nn.Module):
    def __init__(self, patch_len:int,patch_num:int,pred_patch_num:int,embedding_size=512, n_heads=8, dropout=0.05, d_layers=1, decomp_kernel=[32], c_out=1,
                conv_kernel=[2, 4], isometric_kernel=[18, 6], device='cuda'):
        super(Seasonal_Prediction, self).__init__()

        # conv_kernel=[2,4]
        # isometric_kernel=[4,2]
        self.mic = nn.ModuleList([MIC(feature_size=embedding_size, n_heads=n_heads,
                                                   decomp_kernel=decomp_kernel,conv_kernel=conv_kernel,
                                                    isometric_kernel=isometric_kernel, device=device,patch_num=patch_num,patch_len=patch_len,pred_patch_num=pred_patch_num)
